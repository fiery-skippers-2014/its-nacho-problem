...
describe RecipesController do 
  ...
  context "parse data from json object" do 

    it "should have a recipe name" do
      expect(controller.fetch_name(result)).to be_an Array
    end

    it "should have name Escargots Bourguignonne" do 
      expect(controller.fetch_name(result)).to include "Escargots Bourguignonne"
    end

    it "should have a recipe id" do 
      expect(controller.fetch_id(result)).to be_a Array 
    end
    ...
    it "should have an componenets array" do 
      expect(controller.fetch_components(result)).to be_an Array 
    end

    it "should include " do 
      expect(controller.fetch_components(result)).to include ["butter", "shallots", "pepper", "pernod", "worcestershire sauce", "escargot", "garlic", "parsley", "lemon", "dry white wine", "salt", "snail shells", "tabasco sauce"]
    end
  end

  # context "#create" do
  #   it "should save single/multiple recipes to the database" do
  #     post :create
  #     expect(Recipe.create(name: controller.fetch_name(result)[0], img_url: controller.fetch_url(result)[0], components: controller.fetch_components(result)[0]), yummly_id: recipe_url_assembler(controller.fetch_id(result)[0])).to change
  # end
